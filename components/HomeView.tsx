...
export const HomeView: React.FC<HomeViewProps> = ({ products, onCategorySelect }) => {
  ...
  return (
    <div className="flex flex-col space-y-8 animate-in fade-in duration-500 pb-24 w-full">
      ...
      {/* Hero Slider — uses opacity fade to avoid overflow issues */}
      {featuredProducts.length > 0 && (
        <div className="relative w-full h-64 md:h-[400px]" style={{ overflow: 'hidden' }}>
          ...
          {/* Slider Indicators */}
          <div className="absolute bottom-4 right-4 flex space-x-2" style={{ zIndex: 10 }}>
            {featuredProducts.map((_, idx) => (
              <button
                key={idx}
                onClick={() => setCurrentSlide(idx)}
                className={`h-2 rounded-full transition-all ${currentSlide === idx ? 'bg-primary w-6' : 'bg-white/50 hover:bg-white w-2'
                  }`}
              />
            ))}
          </div>
